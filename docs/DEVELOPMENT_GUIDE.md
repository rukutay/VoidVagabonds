...
- **AI action mode enum**: `AAIShipController` includes `EActionMode` values `Idle`, `Moving`, `Following`, `Patroling`, `Fight`, and `Flee` for controller-level action state selection.
- **AI action helpers**:
...
- **NavStaticBig signature tracing**: `UNavigationSubsystem::FindLineTraceNavStaticBigObstacle` keeps traces from the ship position to the target/anchor, adds all cached `SignatureSphere` components to ignored trace components, and filters signature hits so atmosphere/signature overlaps do not hide real body/mesh blockers.
...
- **Blueprint ship state flags**: `AShip` exposes runtime flags for BP/UI logic: `isMoving` (speed threshold), `isHalfSpeed` (current speed > half potential max speed), and `IsPlayerLook` (player controls ship or has an attached spectator look-at).
...
## Debugging / Profiling
- Debug drawing in `#if !UE_BUILD_SHIPPING` with toggle gates.
- Ship navigation debug: `bDrawNavPath` draws/labels all global anchors (cyan = upcoming/current, silver = passed).
- Toggleable logs exist for unstuck checks, steering source/heading, and nav target/avoidance decisions.

## Contribution Workflow
- Branch strategy and commit conventions are not yet standardized for this repository.
