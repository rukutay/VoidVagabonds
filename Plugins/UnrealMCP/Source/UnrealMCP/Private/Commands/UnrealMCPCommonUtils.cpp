                                     ...
                                     const TSharedPtr<FJsonValue>& Value, FString& OutErrorMessage)
{
    ...
    if (Value->Type == EJson::String)
    {
        const TCHAR* ImportResult = Property->ImportText_Direct(*Value->AsString(), PropertyAddr, Object, 0);
        if (ImportResult != nullptr)
        {
            UE_LOG(LogTemp, Display, TEXT("Imported text for property %s: %s"), *PropertyName, *Value->AsString());
            return true;
        }
        OutErrorMessage = FString::Printf(TEXT("Failed to import text for property %s"), *PropertyName);
    }
    else
    {
