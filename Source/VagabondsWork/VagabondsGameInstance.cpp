...
void UVagabondsGameInstance::OnStart()
{
	...
	if (UWorld* World = GetWorld())
	{
		...
		World->GetTimerManager().SetTimer(
			...
			this,
			&UVagabondsGameInstance::RefreshMovingStaticObstacles,
...
void UVagabondsGameInstance::SetRuntimeNavObstacleActors(const TArray<AActor*>& Actors)
{
...
const TArray<FNavObstacleSphereProxy>& UVagabondsGameInstance::GetStaticNavObstacles() const
{
	return CombinedNavObstacles;
}

bool UVagabondsGameInstance::GetClosestObstacleToSegment(const FVector& A, const FVector& B, int32& OutIndex) const
{
	OutIndex = INDEX_NONE;
	return false;
}

bool UVagabondsGameInstance::IsSegmentClearOfStaticObstacles(const FVector& A, const FVector& B, int32* OutFirstHitIndex) const
{
	if (OutFirstHitIndex)
	{
...
const FNavObstacleSphereProxy* UVagabondsGameInstance::FindObstacleByActor(AActor* InActor) const
{
	if (!IsValid(InActor))
	{
		return nullptr;
...
bool UVagabondsGameInstance::DebugTestSegmentAgainstStaticObstacles(const FVector& A, const FVector& B)
{
	...
	if (bNavDebugDrawStatic)
	{
		...
		if (World)
		{
			constexpr float DebugDuration = 1.0f;
...
TArray<FVector> UVagabondsGameInstance::FindGlobalPathAnchors(const FVector& Start, const FVector& Goal) const
{
...
void UVagabondsGameInstance::InitializeNavObstacles()
{
...
void UVagabondsGameInstance::RefreshMovingStaticObstacles()
{
	constexpr float GoldenAngle = 2.39996322972865332f;
...
void UVagabondsGameInstance::RefreshCombinedNavObstacles()
{
