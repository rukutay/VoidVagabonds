...
bool UNavigationSubsystem::IsSegmentClearOfStaticObstacles(const FVector& A, const FVector& B, int32* OutFirstHitIndex) const
{
	if (OutFirstHitIndex)
	{
...
const FNavObstacleSphereProxy* UNavigationSubsystem::FindObstacleByActor(AActor* InActor) const
{
	if (!IsValid(InActor))
	{
		return nullptr;
